...
app.config.update(
    ...
    MAIL_PORT = '465',
    MAIL_USE_SSL = True,
    MAIL_USERNAME = 'gplicbbk@gmail.com',
    MAIL_PASSWORD = '8299158334'
)
mail = Mail(app)
...
def home():
    ...
    return render_template('index.html', params=params, post=post, prev=prev, next=next, page=page, last=last)
...
def edit(sno):
    if ("user" in session and session['user']==params['admin_user']):
        if request.method=="POST":
            ...
            if sno=='0':
                post = Posts(title=box_title, tag_line=tagline, slug=slug, content=content, img_file=img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
